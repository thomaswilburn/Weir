...
<opml version="1.0">
  ...
  for (var i = 0; i < feeds.length; i++) {
    ...
    output += `<outline 
      text="${escape(feed.title)}
      type="rss"
      xmlUrl="${escape(feed.url)}"
      htmlUrl="${escape(feed.site_url)}"/>\n`;
  }
  output += `</body></opml>`;
  req.replyDirect({
    headers: {
